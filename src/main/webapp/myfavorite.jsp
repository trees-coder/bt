...
<script>
    ...
    function load(uid,currentPage) {
        $.get("personal/findFavoriteByPage",{uid:uid,currentPage:currentPage},function (pb) {
            ...
            for (var i = 0;i< pb.list.length;i++){
                ...
                var favorite_div =
                    '<div class="col-md-3">'+
                    '<a href="route_detail.jsp?cid='+route.cid+'&rid='+route.rid+'">\n' +
                    '<img src="'+route.rimage+'" >\n' +
                    '<div class="has_border">\n' +
                    '<h3>'+route.rname+'</h3>\n' +
                    '<div class="price">网付价<em>￥</em><strong>'+route.price+'</strong><em>起</em></div>\n' +
                    '</div></a></div>';
                route_div += favorite_div;
            }
            $("#favoritediv").html(route_div);
            //定位到页面顶部
            window.scrollTo(0,0);
        });
    }
